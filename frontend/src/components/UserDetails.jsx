        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-sm mx-4">
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <button
                        onClick={onClose}
                        className="p-1.5 rounded-lg hover:bg-red-700 text-slate-400 hover:text-white transition"
                    >
                ...
                <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <div>
                        <p className="text-xs text-slate-500 uppercase tracking-widest mb-1">Age</p>
                        <p className="text-sm text-slate-100 font-medium">{user.age || '—'}</p>
                    </div>
                    <div>
                        <p className="text-xs text-slate-500 uppercase tracking-widest mb-1">Gender</p>
                        <p className="text-sm text-slate-100 font-medium">{user.gender || '—'}</p>
                    </div>
                    <div>
                        <p className="text-xs text-slate-500 uppercase tracking-widest mb-1">Country</p>
                        <p className="text-sm text-slate-100 font-medium">{user.country || '—'}</p>
                    </div>
                </div>
